...
function showToast(message, type='success'){
  let box = document.getElementById('toastMessage');
  if(!box){
...
async function imageToDataURLDirect(file){
  if(!file) return '';
  return await fileToDataURL(file);
}

async function saveAndRenderImageData(d, successMessage){
  saveData(d);
  if(supabaseOnline){
    try{
      await pushDataToSupabase(mergeData(d));
    }catch(e){
      console.warn('Supabase no guardó, pero quedó local:', e.message);
      showToast('Imagen cargada localmente. Revisa conexión Supabase.', 'warning');
      render();
      return;
    }
  }
  render();
  showToast(successMessage || 'Carga exitosa.');
}
...
function setup(){$('standingSerieSelect')?.addEventListener('change',renderStandings); document.querySelectorAll('.serie-pill').forEach(btn=>btn.addEventListener('click',()=>{$('standingSerieSelect').value=btn.dataset.serie; document.querySelector('#posiciones')?.scrollIntoView({behavior:'smooth'}); renderStandings()}));
 ...
 $('saveMatch')?.addEventListener('click',async()=>{
  const d=getData();
  let logo=(matchLogoUrl.value||'').trim();
  if(matchLogoFile.files && matchLogoFile.files[0]){
    logo=await imageToDataURLDirect(matchLogoFile.files[0]);
  }
  d.nextMatch={rival:matchRival.value,logo,serie:matchSerie.value,date:matchDate.value,hour:matchHour.value,place:matchPlace.value};
  await saveAndRenderImageData(d, 'Próximo partido actualizado exitosamente.');
});
 ...
 $('addPresidentGallery')?.addEventListener('click',async()=>{
  const d=getData();
  let image=(presidentGalleryUrl.value||'').trim();
  if(presidentGalleryFile.files && presidentGalleryFile.files[0]){
    image=await imageToDataURLDirect(presidentGalleryFile.files[0]);
  }
  if(!presidentGalleryName.value.trim()) return showToast('Falta el nombre del presidente.', 'error');
  if(!image) return showToast('Debes seleccionar una fotografía o pegar una URL.', 'error');
  d.presidents.push({name:presidentGalleryName.value.trim(),period:presidentGalleryPeriod.value.trim(),image});
  presidentGalleryName.value='';
  presidentGalleryPeriod.value='';
  presidentGalleryUrl.value='';
  presidentGalleryFile.value='';
  await saveAndRenderImageData(d, 'Presidente cargado exitosamente.');
});
 ...
 $('addNews')?.addEventListener('click',async()=>{
  if(!newsTitle.value.trim()||!newsText.value.trim()) return showToast('Completa título y texto de la noticia.', 'error');
  const d=getData();
  let image=(newsImageUrl.value||'').trim();
  if(newsImageFile.files && newsImageFile.files[0]){
    image=await imageToDataURLDirect(newsImageFile.files[0]);
  }
  d.news.unshift({title:newsTitle.value.trim(),text:newsText.value.trim(),date:new Date().toLocaleDateString('es-CL'),image});
  newsTitle.value='';
  newsText.value='';
  newsImageUrl.value='';
  newsImageFile.value='';
  await saveAndRenderImageData(d, 'Noticia cargada exitosamente.');
});
 ...
 $('addSponsor')?.addEventListener('click',async()=>{
  const d=getData();
  let url=(sponsorUrl.value||'').trim();
  if(sponsorFile.files && sponsorFile.files[0]){
    url=await imageToDataURLDirect(sponsorFile.files[0]);
  }
  if(!sponsorName.value.trim()||!url) return showToast('Agrega nombre e imagen del auspiciador.', 'error');
  d.sponsors.push({name:sponsorName.value.trim(),url});
  d.metrics.sponsors=String(d.sponsors.length);
  sponsorName.value='';
  sponsorUrl.value='';
  sponsorFile.value='';
  await saveAndRenderImageData(d, 'Auspiciador cargado exitosamente.');
});
 ...
 $('addMedia')?.addEventListener('click',async()=>{
  if(!mediaTitle.value.trim()) return showToast('Agrega un título.', 'error');
  const d=getData();
  let url=(mediaUrl.value||'').trim();
  let type=mediaType.value;
  if(mediaFile.files && mediaFile.files[0]){
    url=await fileToDataURL(mediaFile.files[0]);
    type=mediaFile.files[0].type.startsWith('video')?'Video':'Foto';
  }
  if(!url) return showToast('Debes adjuntar archivo o pegar URL.', 'error');
  d.media.unshift({title:mediaTitle.value.trim(),type,url});
  mediaTitle.value='';
  mediaUrl.value='';
  mediaFile.value='';
  await saveAndRenderImageData(d, 'Foto/video cargado exitosamente.');
});
